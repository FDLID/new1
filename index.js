...
⌬ Limit: ${limitawal-updated.limit}`
		...
		if (!isGroup) return reply(ind.groupo())
               ...
               case 'tercantik':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					case 'Sayang':
					case 'sayang':
					client.sendMessage(from, 'iya sayangqu ada apa? 🥰',MessageType.text, { quoted: mek} )
					break
				case 'slap':
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						if (args.length < 1) return client.sendMessage(from, '𝘂𝘀𝗲r 𝗻𝗮𝗺𝗲 𝗺𝗮𝗻𝗮 ?', text, {quoted: mek})
           ...
           case 'demote':
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗴𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝘄𝗲𝗹𝗰𝗼𝗺𝗲/𝗹𝗲𝗳𝘁 ??𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗼𝗻𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝘄𝗲𝗹𝗰𝗼??𝗲/𝗹𝗲𝗳?? 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️')
					} else {
						reply(ind.satukos())
					}
					break 
				...
				case 'tagall':
					...
					mentions('╔══🜲〘 OTAG ALL 〙🜲══'+teks+'╚═〘 *FDL BOT* 〙', members_id, true)
					break
				case 'delete':
				case 'del':
				case 'd':
				...
				case 'antilink':
					if (!isGroup) return reply(ind.groupo())
                    if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('MyBoo :𝘃')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*SUDAH AKTIF* !!!')
						antilink.push(from)
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('*❬ Sukses ❭ Meng-aktifkan Features ANTILINK*')
					} else if (Number(args[0]) === 0) {
						antilink.splice(from, 1)
						fs.writeFileSync('./database/group/antilink.json', JSON.stringify(antilink))
						reply('*❬ Sukses ❭ Meng-nonaktfikan Features ANTILINK*')
					} else {
						reply(ind.satukos())
					}
					break
		...
		if (budy.includes(`makasih`)) {
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  reply(`Maaf to Sorry *${pushname}*, Command *${prefix}${command}* Nggk Ada Di Dalam Daftar *${prefix}menu* 🙂`)
                  }
