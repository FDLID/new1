...
const vcard = 'BEGIN:VCARD\n'  // Jangan di ubah biar ga error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=628990542731:+628990542731\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
async function starts() {
    ...
    fs.existsSync('./lolhuman.json') && fdl.loadAuthInfo('./lolhuman.json')
    if (apikey == "") {
   ...
   if (cr == "") {
    ...
    fs.writeFileSync('./lolhuman.json', JSON.stringify(fdl.base64EncodedAuthInfo(), null, '\t'))

	fdl.on('group-participants-update', async(chat) => {
        try {
            mem = chat.participants[0]
            try {
                var pp_user = await fdl.getProfilePicture(mem)
            } catch (e) {
                var pp_user = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
            }
            if (chat.action == 'add') {
                ini_user = fdl.contacts[mem]
                ini_img = await getBuffer(`http://api.lolhuman.xyz/api/welcomeimage?apikey=${apikey}&img=${pp_user}&text=${ini_user.notify}`)
                group_info = await fdl.groupMetadata(chat.jid)
                welkam = `${ini_user.notify}, Welkam to ${group_info.subject}`
                fdl.sendMessage(chat.jid, ini_img, MessageType.image, { caption: welkam })
            }
            if (chat.action == 'remove') {
                ini_user = fdl.contacts[mem]
                ini_out = `${ini_user.notify}, Sayonara👀👋`
                fdl.sendMessage(chat.jid, ini_out, MessageType.text)
            }
        } catch (e) {
            console.log('Error :', e)
        }
    ...
    fdl.on('chat-update', async(lol) => {
        try {
            const time = moment.tz('Asia/Jakarta').format('HH:mm:ss')
            if (!fdl.hasNewMessage) return
            lol = JSON.parse(JSON.stringify(lol)).messages[0]
            if (!fdl.message) return
            if (fdl.key && fdl.key.remoteJid == 'status@broadcast') return
            if (fdl.key.fromMe) return
            global.prefix
	    const ownerNumber = ["628990542731@s.whatsapp.net"] // owner number ubah aja
            const content = JSON.stringify(fdl.message)
            const from = fdl.key.remoteJid
            const type = Object.keys(fdl.message)[0]
            const insom = from.endsWith('@g.us')
            const nameReq = insom ? fdl.participant : fdl.key.remoteJid
            pushname2 = fdl.contacts[nameReq] != undefined ? fdl.contacts[nameReq].vname || fdl.contacts[nameReq].notify : undefined

            const { text, extendedText, contact, location, liveLocation, image, video, sticker, document, audio, product } = MessageType

            body = (type === 'conversation' && fdl.message.conversation.startsWith(prefix)) ? fdl.message.conversation : (type == 'imageMessage') && fdl.message.imageMessage.caption.startsWith(prefix) ? fdl.message.imageMessage.caption : (type == 'videoMessage') && fdl.message.videoMessage.caption.startsWith(prefix) ? fdl.message.videoMessage.caption : (type == 'extendedTextMessage') && fdl.message.extendedTextMessage.text.startsWith(prefix) ? fdl.message.extendedTextMessage.text : ''
            budy = (type === 'conversation') ? fdl.message.conversation : (type === 'extendedTextMessage') ? fdl.message.extendedTextMessage.text : ''
            var Link = (type === 'conversation' && fdl.message.conversation) ? fdl.message.conversation : (type == 'imageMessage') && fdl.message.imageMessage.caption ? fdl.message.imageMessage.caption : (type == 'videoMessage') && fdl.message.videoMessage.caption ? fdl.message.videoMessage.caption : (type == 'extendedTextMessage') && fdl.message.extendedTextMessage.text ? fdl.message.extendedTextMessage.text : ''
	    ...
	    const sender = isGroup ? fdl.participant : fdl.key.remoteJid
	    ...
	    const isPrem = premium.includes(sender)
            ...
            const reply = (teks) => {
                fdl.sendMessage(from, teks, text, { quoted: lol})
            }
	   	...
	   	fdl.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` }}})
            ...
            const mentions = (teks, memberr, id) => {
                (id == null || id == undefined || id == false) ? fdl.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }): fdl.sendMessage(from, teks.trim(), extendedText, { quoted: lol, contextInfo: { "mentionedJid": memberr } })
            }
		...
		var title = (cr)
                ...
                fdl.sendMessage(from, teks, text, ini_cstoko)
            }
 ...
 if (isCmd) cmdadd()
		    ...
		    myMonths = ["Januari","Februari","Maret","April","Mei","Juni","Juli","Agustus","September","Oktober","November","Desember"];
                ...
                const tanggal = `${thisDay}, ${day} - ${myMonths[bulan]} - ${year}`
                 ...
                 fdl.sendMessage(from, help(prefix, pushname2,  premi, uptime, tanggal, pepolu, hayuk), text, ini_csreply)
                    break
                    case 'clearall':
                    if (sender.split("@")[0] != owner) return reply("Command only for owner bot")
                    list_chat = await fdl.chats.all()
                    for (let chat of list_chat) {
                        fdl.modifyChat(chat.jid, "delete")
                    }
                    reply("success clear all chat")
                    break
		case 'sticker2': 
				if (!isRegistered) return reply(ind.noregis())
					if ((isMedia && !fdl.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata(namo, ator)} ${ran} -o ${ran}`, async (error) => {
									//if (error) {
											...
											// fs.unlinkSync(ran)
											// }
									fdl.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: lol})
									fs.unlinkSync(media)	
									fs.unlinkSync(ran)	
								})
							})
							...
							.toFormat('webp')
							.save(ran)
					} else if ((isMedia && fdl.message.videoMessage.seconds < 11 || isQuotedVideo && fdl.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) && args.length == 0) {
		...
		case 'tagimg':
                    ...
                    if ((isMedia && !fdl.message.videoMessage || isQuotedImage) && args.length == 0) {
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(lol).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : lol
                        filePath = await fdl.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
                        var value = args.join(" ")
                        ...
                        fdl.sendMessage(from, ini_buffer, image, options)
                ...
                case 'tagstick':
                    if (sender.split("@")[0] != owner) return reply("Command only for owner bot")
                    if ((isMedia && !fdl.message.videoMessage || isQuotedSticker) && args.length == 0) {
                        ...
                        ini_buffer = fs.readFileSync(filePath)
                        fdl.sendMessage(from, ini_buffer, sticker, options)
                        fs.unlinkSync(filePath)
                    } else {
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    fdl.sendMessage(from, ini_buffer, video, { quoted: lol})
                    break
			case 'setpp': 
                        if (!isGroup) return reply(ind.groupo())
                       if (!isGroupAdmins) return reply(ind.admin())
                        if (!isBotGroupAdmins) return reply('BOT HARUS JADI ADMIN DULU')
                       media = await fdl.downloadAndSaveMediaMessage(lol)
           	...
           	case 'setname':
                ...
                fdl.sendMessage(from, 'Succes, Ganti Nama Grup', text, {quoted: lol})
					break
                case 'setdesc':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
                ...
                fdl.sendMessage(from, 'Succes, Ganti Deskripsi Grup', text, {quoted: lol})
					break
           				case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply('BOT HARUS JADI ADMIN DULU')
					if (fdl.message.extendedTextMessage === undefined || fdl.message.extendedTextMessage === null) return reply('𝗧𝗮𝗴 𝘁𝗮𝗿𝗴𝗲𝘁 𝘆𝗮𝗻𝗴 𝗶𝗻𝗴𝗶𝗻 𝗱𝗶 𝘁𝗲𝗻𝗱𝗮𝗻𝗴!')
					mentioned = fdl.message.extendedTextMessage.contextInfo.mentionedJid
				...
				case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply('BOT HARUS JADI ADMIN DULU')
					if (fdl.message.extendedTextMessage === undefined || fdl.message.extendedTextMessage === null) return reply('𝗧𝗮?? 𝘁𝗮𝗿𝗴𝗲𝘁 𝘆𝗮𝗻𝗴 𝗶𝗻𝗴𝗶𝗻 𝗱𝗶 𝘁𝗲𝗻𝗱𝗮𝗻𝗴!')
					mentioned = fdl.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
					...
					if (fdl.message.extendedTextMessage === undefined || fdl.message.extendedTextMessage === null) return reply('𝗧𝗮𝗴 𝘁𝗮𝗿𝗴𝗲𝘁 ??𝗮𝗻𝗴 𝗶𝗻𝗴𝗶𝗻 𝗱𝗶 𝘁𝗲𝗻𝗱𝗮𝗻𝗴!')
					mentioned = fdl.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
		...
		case 'antivirtek':
					...
					if (Number(args[0]) === 1) {
						...
						fdl.sendMessage(from,`ALLERT!!! DILARANG KIRIM VIRUS KALO GAMAU DI KICK GRUP INI ANTI VIRUS`, text)
					...
					fdl.sendMessage(from, { quoted: lol})
		...
		case 'antilinkgrup':
					...
					if (Number(args[0]) === 1) {
						...
						fdl.sendMessage(from,`ALLERT!!! Jika bukan admin jangan kirim link grup`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntiLink) return reply('EMANG AKTIF?')
		...
		case 'daftar':
                ...
                try {
		pp_user = await fdl.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
				} catch {
                ...
                if (isGroup) {
                    ...
                    await fdl.sendMessage(from, daftarpp, image, {quoted: lol, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
		...
		truteh = await getBuffer(`https://i.ibb.co/Gp4H47k/7dba54f7e250.jpg`)
		...
		tod = await getBuffer(`https://i.ibb.co/Gp4H47k/7dba54f7e250.jpg`)
		fdl.sendMessage(from, tod, image, { quoted: lol, caption: '*DARE*\n\n'+ der })
		break
		case 'elang':
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    ini_url = ini_url.result
                    ini_buffer = await getBuffer(ini_url)
                    fdl.sendMessage(from, ini_buffer, image, { quoted: lol})
                    break
		//premiom
		case 'premlist':
				if (!isRegistered) return reply(ind.noregis())
					...
					fdl.sendMessage(from, teks.trim(), extendedText, {quoted: lol, contextInfo: {"mentionedJid": premium}})
					break
					case 'listpenyimak': 
        		let ido = args && /\d+\-\d+@g.us/.test(args[0]) ? args[0] : from
			    let online = [...Object.keys(fdl.chats.get(ido).presences), fdl.user.jid]
			    fdl.sendMessage(from, '*CIE NYIMAK AJE LU*\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n` + `\n*©POWERED BY FDL*`, text, { quoted: lol,
		...
		case 'spekhp':
	            query = args.join(" ")
                    ...
                    fdl.sendMessage(from, buffer, image, { quoted: lol, caption: txt })
                    break
		case 'togif':
                    if ((isMedia && !fdl.message.videoMessage || isQuotedSticker)) {
                        const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(lol).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : lol
		...
		case 'ocr':
                    ...
                    if ((isMedia && !fdl.message.videoMessage || isQuotedImage) && args.length == 0) {
                        ...
                        var filePath = await fdl.downloadAndSaveMediaMessage(encmedia, filename = getRandom());
		...
		case 'delete':
			...
			if (!isGroup) return reply('KHUSUS GROUP')
                       ...
                       if (sender.split("@")[0] != owner) return reply("Command only for owner bot")
					fdl.deleteMessage(from, { id: fdl.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
					break
		case 'leave':
			if (sender.split("@")[0] != owner) return reply("Command only for owner bot")
                      setTimeout( () => {
                      ...
                      setTimeout( () => {
                      fdl.updatePresence(from, Presence.composing) 
                      fdl.sendMessage(from, 'SaYoNaRa:/', text)
                      }, 0)
                      break			
		case 'takestick':
		if (!isRegistered) return reply(ind.noregis())
                    if ((isMedia && !fdl.message.videoMessage || isQuotedSticker)) {
                        ...
                        request({
                            ...
                            fs.writeFileSync(file_name, body, "binary")
                            ini_buff = fs.readFileSync(file_name)
                            fdl.sendMessage(from, ini_buff, sticker, { quoted: lol })
                            fs.unlinkSync(file_name)
                        });
                    } else {
                        reply(`Kirim gambar dengan caption ${prefix + command} atau tag gambar yang sudah dikirim`)
            ...
            case 'creator':
		if (!isRegistered) return reply(ind.noregis())
                fdl.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: lol})
                fdl.sendMessage(from, 'Tuh kontak Mr.A43G <•_•> ,Jangan Diubah Ya Kontaknya (*>*)',MessageType.text, { quoted: lol} )
	        ini_buffer = await getBuffer("https://i.ibb.co/tqX1xmn/20210326-134340.jpg")
                fdl.sendMessage(from, ini_buffer, MessageType.image, {quoted: lol, caption: '*SUBSCRIBE:*\n*https://youtube.com/channel/UC85BV5PuFNdhEF1JIR6mKjw*'})
                 break
				case 'addprem':
				if (!isRegistered) return reply(ind.noregis())
					if (sender.split("@")[0] != owner) return reply("Command only for owner bot")
		...
		case 'twtstalk':
				if (!isRegistered) return reply(ind.noregis())
				username = args[0]
					get_result = await fetchJson(`http://lolhuman.herokuapp.com/api/twitter/${username}?apikey=RiuCok`, {method: 'get'})
					get_result = get_result.result
					...
					fdl.sendMessage(from, profile, image, {quoted: lol, caption: txt})
					fdl.sendMessage(from, banner, image, {quoted: lol, caption: 'BANNERNYA OM!' })
					break
		case 'toimg':
				if (!isRegistered) return reply(ind.noregis())
				if (!isQuotedSticker) return reply('Reply/tag sticker !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						fdl.sendMessage(from, buffer, image, {quoted: lol, caption: 'tuh dh jadi '})
						fs.unlinkSync(ran)
					})
					break
		case 'toimg2':
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    if ((isMedia && !fdl.message.videoMessage || isQuotedSticker) && args.length == 0) { 
                        const encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(lol).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : lol
                        filePath = await fdl.downloadAndSaveMediaMessage(encmedia)
                        ...
                        request({
                            ...
                            fs.writeFileSync(file_name, body, "binary")
                            ini_buff = fs.readFileSync(file_name)
                            fdl.sendMessage(from, ini_buff, image, { quoted: lol, caption: 'Ni Udh Jadi Boss' })
                            fs.unlinkSync(file_name)
                        });
                    } else {
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    break
			case 'superhero':
                    get_result = await fetchJson(`http://lolhuman.herokuapp.com/api/superhero?apikey=RiuCok&query=${body.slice(11)}`)
                    get_result = get_result.result
		...
		case 'getpic':
					...
					if (fdl.message.extendedTextMessage === undefined || fdl.message.extendedTextMessage === null) return reply('Siap Boss')
					mentioned = fdl.message.extendedTextMessage.contextInfo.mentionedJid[0]
						try {
						pp = await fdl.getProfilePicture(mentioned)
						buffer = await getBuffer(pp)
						
						fdl.sendMessage(from, buffer, image, {quoted: lol})
					} catch (e) {
						fdl.sendMessage(from, buffer, image, {quoted:lol})
					}
					break
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, ini_buffer, audio, { quoted: lol, mimetype: Mimetype.mp4Audio })
                    break
                case 'asmaulhusna':
		if (!isRegistered) return reply(ind.noregis())
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, ini_buffer, image, { quoted: lol, caption: ini_txt })
                    get_audio = await getBuffer(get_result.audio[3].link)
                    fdl.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_info.title}.mp3`, quoted: lol})
                    break
                case 'ytplay2':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} Melukis Senja`)
                    ...
                    fdl.sendMessage(from, ini_buffer, image, { quoted: lol, caption: get_result.title })
                    get_audio = await getBuffer(get_result.audio)
                    fdl.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_result.title}.mp3`, quoted: lol})
                    get_video = await getBuffer(get_result.video)
                    fdl.sendMessage(from, get_video, video, { mimetype: Mimetype.mp4, filename: `${get_result.title}.mp4`, quoted: lol})
                    break
		    case 'ytmp33':
			if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
                    ini_link = args[0]
                    get_result = await fetchJson(`http://lolhuman.herokuapp.com/api/ytaudio2?apikey=RiuCok&url=${ini_link}`)
                    get_result = get_result.result
                    txt = `Title : ${get_result.title}\n`
                    buffer = await getBuffer(get_result.thumbnail)
                    fdl.sendMessage(from, buffer, image, { quoted: lol, caption: txt })
		    get_audio = await getBuffer(get_result.link[0].size)
		    fdl.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.title}.mp3`, quoted: lol })
                    break
                case 'ytsearch':
			if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} Melukis Senja`)
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: lol})
                    break
                case 'ytmp42':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} https://www.youtube.com/watch?v=qZIQAk-BUEc`)
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    for (sticker_ in ini_sticker) {
                        ...
                        fdl.sendMessage(from, ini_buffer, sticker)
                    }
                    break
		case 'donate':
		if (!isRegistered) return reply(ind.noregis())
                var punya_wa = "0@s.whatsapp.net"
                    ...
                    fdl.sendMessage(from, donate(pushname2), text, ini_csreplythum)
                    break
                case 'tiktoknowm':
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: lol})
                    break
                case 'spotify':
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, thumbnail, image, { quoted: lol, caption: ini_txt })
                    get_audio = await getBuffer(get_result.link[3].link)
                    fdl.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.title}.mp3`, quoted: lol})
                    break
                case 'spotifysearch':
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.info.song}.mp3`, quoted: lol})
                    break
                case 'igdl':
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    ini_buffer = await getBuffer(ini_url)
                    fdl.sendMessage(from, ini_buffer, ini_type, { quoted: lol})
                    break
                case 'fbdl':
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    if ((isMedia && !fdl.message.videoMessage || isQuotedImage) && args.length == 0) {
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(lol).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : lol
                        const filePath = await fdl.downloadAndSaveMediaMessage(encmedia, filename = getRandom());
                        ...
                        fdl.sendMessage(from, ini_video, video, { quoted: lol, caption: ini_txt })
                    } else {
                        reply(`Kirim gambar dengan caption ${prefix + command} atau tag gambar yang sudah dikirim`)
                    }
                    break
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, ini_buffer, document, { quoted: lol, mimetype: Mimetype.pdf, filename: `${henid}.pdf` })
                    break
                case 'nhentaisearch':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} Gotoubun No Hanayome`)
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, ini_icon, image, { quoted: lol, caption: ini_txt })
                    break
                case 'wikipedia':
		if (!isRegistered) return reply(ind.noregis())
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, get_buffer, image, { quoted: lol, caption: ini_txt })
                    break
                case 'lirik':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} Melukis Senja`)
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    fdl.sendMessage(from, { degreesLatitude: get_result.latitude, degreesLongitude: get_result.longitude }, location, { quoted: lol})
                    reply(ini_txt)
                    break
                case 'covidindo':
		if (!isRegistered) return reply(ind.noregis())
		...
		if (!isRegistered) return reply(ind.noregis())
                ...
                case 'quotesimage':
		if (!isRegistered) return reply(ind.noregis())
		buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/random/quotesimage?apikey=RiuCok`)
		fdl.sendMessage(from, buffer, image, { quoted: lol})
		break
                case 'faktaunik':
                case 'katabijak':
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    for (var x = 0; x <= 5; x++) {
                        ...
                        fdl.sendMessage(from, ini_buffer, image)
                    }
                    break
                case 'konachan':
		if (!isRegistered) return reply(ind.noregis())
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                ...
                case 'artinama':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} FDL`)
                    ini_nama = args.join(" ")
		...
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    if ((isMedia && !fdl.message.videoMessage || isQuotedImage)) {
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(lol).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : lol
                        filePath = await fdl.downloadAndSaveMediaMessage(encmedia, filename = getRandom());
                        file_name = getRandom(".webp")
                        ini_txt = args.join(" ").split("|")
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    exec(`wget "${ini_buffer}" -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                        ...
                        fdl.sendMessage(from, buff, sticker, { quoted: lol})
                        fs.unlinkSync(rano)
                    })
                    break
                case 'wasted':
		...
		if (!isRegistered) return reply(ind.noregis())
                    ...
                    fdl.sendMessage(from, ini_buffer, sticker, { quoted: lol})
                    break
                case 'fakedonald':
		if (!isRegistered) return reply(ind.noregis())
                    if (args.length == 0) return reply(`Example: ${prefix + command} FDL`)
                ...
                case 'faketoko':
		if (!isRegistered) return reply(ind.noregis())
                    await faketoko(teks = "Tahu Bacem", url_image = "https://i.ibb.co/JdfQ73m/photo-2021-02-05-10-13-39.jpg", title = "FDL", code = "IDR", price = 1000000)
                    break
    ...
    case 'ttp':
		...
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, buffer, sticker, { quoted: lol})
                    break
                    case 'qrcode':
		if (!isRegistered) return reply(ind.noregis())
		...
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    if (args.length == 0) return reply(`Usage: ${prefix + command} query\nExample: ${prefix + command} FDL api.lolhuman.xyz`)
		username = args[0]
		comment = args[2]
		...
		if (!isRegistered) return reply(ind.noregis())
		if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    if (args.length == 0) return reply(`Usage: ${prefix + command} nik|provinsi|kabupaten|nama|tempat, tanggal lahir|jenis kelamin|jalan|rt/rw|kelurahan|kecamatan|agama|status nikah|pekerjaan|warga negara|berlaku sampai|url_image\n\nExample: ${prefix + command} 456127893132123|bumipertiwi|fatamorgana|FDL|mars, 99-99-9999|belum ditemukan|jl wardoyo|999/999|turese|imtuni|alhamdulillah islam|jomblo kack|mikirin dia|indo ori no kw|hari kiamat|https://i.ibb.co/Xb2pZ88/test.jpg`)
		...
		if (!isRegistered) return reply(ind.noregis())
			...
			//Stalking
                   case 'igstalk':
					if (!isRegistered) return reply(ind.noregis())
					get_result = await fetchJson(`http://lolhuman.herokuapp.com/api/stalkig/${body.slice(9)}?apikey=RiuCok`, {method: 'get'})
					get_result = get_result.result
		...
		case 'githubstalk':
					if (!isRegistered) return reply(ind.noregis())
					get_result = await fetchJson(`http://lolhuman.herokuapp.com/api/github/${body.slice(13)}?apikey=RiuCok`, {method: 'get'})
					get_result = get_result.result
		...
		case 'tebakbendera':
					...
					setTimeout( () => {
					fdl.sendMessage(from, '*➸ Jawaban :* '+anu.result.name, text, {quoted: lol}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					fdl.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					...
					setTimeout( () => {
					fdl.sendMessage(from, tebakbender, text, {quoted: lol}) // ur cods
					}, 0) // 1000 = 1s,
					break 
                case 'tebakgambar':
					if (!isRegistered) return reply(ind.noregis())
					...
					setTimeout( () => {
					fdl.sendMessage(from, '*➸ Jawaban :* '+anu.result.answer, text, {quoted: lol}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					fdl.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					fdl.sendMessage(from, bufferkkk, image, { caption: '_Jelaskan Apa Maksud Gambar Ini_', quoted: lol}) // ur cods
					}, 0) // 1000 = 1s,
					break  
				case 'family100':
					if (!isRegistered) return reply(ind.noregis())
					...
					setTimeout( () => {
					fdl.sendMessage(from, '*➸ Jawaban :* '+anu.result.aswer, text, {quoted: lol}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					fdl.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					fdl.sendMessage(from, family, text, {quoted: lol}) // ur cods
					}, 0) // 1000 = 1s,
					break
					case 'caklontong':
					if (!isRegistered) return reply(ind.noregis())
					...
					setTimeout( () => {
					fdl.sendMessage(from, caklontong, text, {quoted: lol}) // ur cods
					}, 0) // 1000 = 1s,
					break 
					case 'asupan':
			if (!isRegistered) return reply(ind.noregis())
			if (!isPrem) return reply('AND BUKAN USER PREM JIKA MAU JADI PREM KETIK #owner')
                    ...
                    fdl.sendMessage(from, buffer, video, { quoted: lol, mimetype: Mimetype.mp4, filename: "asupan.mp4" })
                    break
					case 'meme': 
				if (!isRegistered) return reply(ind.noregis())
				...
				case 'darkjoke': 
				if (!isRegistered) return reply(ind.noregis())
				buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/meme/darkjoke?apikey=RiuCok`, {method: 'get'})
				fdl.sendMessage(from, buffer, image, {quoted: lol, caption: 'FIX BUG BY FDL'})
				break
				case 'namaninja': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: Anjay`)
		...
		if (budy.includes("://chat.whatsapp.com/")){
		        ...
		        setTimeout( () => {
			        fdl.groupRemove(from, [kic]).catch((e)=>{reply(`*BOT HARUS JADI ADMIN!*`)})
		        }, 3000)
		        setTimeout( () => {
			        fdl.updatePresence(from, Presence.composing)
			        reply("Hedsot....")
		        ...
		        setTimeout( () => {
                    ...
                    if (!isGroup && !isCmd) {
                        await fdl.updatePresence(from, Presence.composing)
